...
=======
    ...
    var onChatSmartReply = function(e) {
        ...
        var obj = {},
            _answer = $(robotAnswer).find('a');
        //type=3 未搜索到智能回复答案 不进行发送
        if(_answer.html() === '' || _answer.attr('answerType') === '3' || user.uid == '')
            return;
    ...
    var onDirectSendBtn = function() {
        ...
        var _answer = $(robotAnswer).find('a');
        //type=3 未搜索到智能回复答案 不进行发送
        if(_answer.html() === '' || _answer.attr('answerType') === '3')
            return;
        //TODO 调取外部接口 直接给用户发送智能回复答案
        // console.log($(robotAnswer).find('a').html());
        $(document.body).trigger('rightside.directsendreply',[{
    ...
    var bindLitener = function() {
        $(document.body).on('leftside.onselected',onLoadUserInfo);
        //加载用户信息 点击左侧用户列表才会触发
        $(document.body).on('scrollcontent.onSearchUserChat',onGetReplyByChat);
        //点击聊天内容获取智能回复
        $(oHomeuser).delegate('#quickSerch','keyup',onSerchContent);
        // $(robotDirectHideBtn).delegate('a.quickSendBtn','click',onDirectSendBtn);
        // $(robotAnswer).delegate('a','click',onSendAnswer);
    ...
    init();
>>>>>>> d5ad409fc4c164d55ed1acac914fe93fe1c6cc8e
