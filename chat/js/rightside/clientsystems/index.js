...
var Client = function(node,core,data){
    ...
    $.ajax({
      ...
      // ];
        //TODO 三种情况  返回值为空  返回值为1  返回值大于1
          ...
          $(drowdownMenu).attr('href','#clientSystem');
          for(var i=0;i<data.length;i++){
              for(var tmp in data[i]){
                  ...
                  $(drowdownMenu).attr('addrurl',data[i][tmp]).attr('data-toggle','tab');
                  $(drowdownMenu).on('click',function(){
                      $(clientSysIframe).attr("src", $(drowdownMenu).attr('addrurl')).ready();
                  });
              }
          }
          }else{
              var parentDom = $(drowdownMenu);
  ...
  var parseDOM = function() {
    ...
    drowdownMenu = $(clientNav).find('.js-dropdown-toggle');
