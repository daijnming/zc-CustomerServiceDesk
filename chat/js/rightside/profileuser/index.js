	...
	var initConfig = function(){
		...
		regExUserInfo = {
			uname:[
				...
				{'regex':/^.{0,100}$/,'alert':'最大输入100字符，请重新输入'}],
			realname:[
				// {'regex':/\S/,alert:'格式错误，不允许为空'},
				{'regex':/^[^<>//]*$/,'alert':'格式错误，请重新输入'},
				{'regex':/^.{0,32}$/,'alert':'最大输入32字符，请重新输入'}],
			tel:[
				// {'regex':/\S/,alert:'格式错误，不允许为空'},
				{'regex':/^[0-9\-#*]*$/,'alert':'只允许数字、“#”、“*”、“-”'},
				{'regex':/(^[0-9\-#*]{8,30}$)|(^[0-9\-#*]{0}$)/,'alert':'输入8-30字符，请重新输入'}],
			birthday:[
				// {'regex':/\S/,alert:'格式错误，不允许为空'},
				{'regex':/^[1-2][0-9]{3}\-(([0-2]{1}[0-9]{1})|(3[0-1]{1}))(\-([0-2]{1}[0-9]{1})|(3[0-1]{1}))$/,'alert':'只允许数字、“-”'}],
			email:[
				// {'regex':/\S/,alert:'格式错误，不允许为空'},
				{'regex':/^[^<>//]*$/,'alert':'只允许字母、数字或下划线'},
				{'regex':/^([a-zA-Z0-9]+([-_\.][a-zA-Z0-9]+)*(?:@(?!-))(?:(?:[a-zA-Z0-9]*)(?:[a-zA-Z0-9](?!-))(?:\.(?!-)))+[a-zA-Z0-9]{2,})$/,'alert':'格式错误，请重新输入'},
				{'regex':/(^[a-zA-Z0-9]{1,20}([-_\.][a-zA-Z0-9]{1,20})*(?:@(?!-))(?:(?:[a-zA-Z0-9]{0,10})(?:[a-zA-Z0-9](?!-))(?:\.(?!-))){1,20}[a-zA-Z0-9]{2,10}$)|(^[0-9]{0}$)/,'alert':'最大输入60字符，请重新输入'}],
			qq:[
				// {'regex':/\S/,alert:'格式错误，不允许为空'},
				{'regex':/^[^<>//]*$/,'alert':'格式错误，请重新输入'},
			...
			weibo:[
				// {'regex':/\S/,alert:'格式错误，不允许为空'},
				{'regex':/^[^<>//]*$/,'alert':'格式错误，请重新输入'},
				{'regex':/(^.{4,24}$)|(^[0-9]{0}$)/,'alert':'输入4-24位字符，请重新输入'}],
			remark:[
				// {'regex':/\S/,alert:'格式错误，不允许为空'},
				{'regex':/^[^<>//]*$/,'alert':'格式错误，请重新输入'},
				{'regex':/^.{0,200}$/,'alert':'最大输入200字符，请重新输入'}]
		};
	};
